...
function publish_docs() {
  ...
  docker run --rm -ti -e CI_GITHUB_USERNAME -e CI_GITHUB_PASSWORD -e CI_JFROG_USERNAME -e CI_JFROG_PASSWORD -e JAVA_HOME=/usr/lib/jvm/java-1.8-openjdk -v $(pwd):/test -v $HOME/.m2:/test/.m2 nodevops/maven-fury-confd:1.0 /opt/local/maven-3.3.x/bin/mvn site -P publish-site --settings maven-settings.xml -Dconfd.local.path.for.tests=/opt/local/confd-0.11.0/confd
...
function run_tests(){
  ...
  docker run --rm -ti -e JAVA_HOME=/usr/lib/jvm/java-1.7-openjdk -v $(pwd):/test -v $HOME/.m2:/test/.m2 nodevops/maven-fury-confd:1.0 /opt/local/maven-3.1.x/bin/mvn clean install -Dconfd.local.path.for.tests=/opt/local/confd-0.11.0/confd
